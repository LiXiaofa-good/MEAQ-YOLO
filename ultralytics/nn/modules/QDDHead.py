...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, bn=True):
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d),
                              groups=g, dilation=d, bias=not bn)
...
class QDDHead(nn.Module):
    ...
    def __init__(
        ...
        self.reg_blocks = nn.ModuleList([
            build_reg_block(reg_block_type, hidc) for _ in ch
        ])

       
        self.quality_proj = nn.ModuleList([
            nn.Conv2d(hidc, 1, 1, bias=True) for _ in ch
        ]) if use_quality_gate else None

        
        self.reg_out = nn.ModuleList([
            nn.Conv2d(hidc, 4 * reg_max, 1, bias=True) for _ in ch
        ])

        
        self.cls_blocks = nn.ModuleList([
            ClsBlock(c_in=c, c_out=clsc,
                     cls_dw_type=cls_dw_type,
                     att_type=att_type,
                     reduction=reduction)
            for c in ch
        ])
        ...
        self.cls_out = nn.ModuleList([
            nn.Conv2d(clsc, nc, 1, bias=True) for _ in ch
        ])
    ...
    def forward(self, x: List[torch.Tensor]):
        ...
        if self.export and getattr(self, "format", None) in (
    ...
    def bias_init(self) -> None:
        for i, (reg_o, cls_o, s) in enumerate(
            zip(self.reg_out, self.cls_out, self.stride)
        ):
            ...
            s_val = float(s)
            cls_o.bias.data[:] = (
                math.log(5.0 / self.nc / (640.0 / s_val) ** 2)
                if s_val > 0 else 0.0
            )
