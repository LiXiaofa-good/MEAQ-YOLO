    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
        elif m is QDDHead:  # QDDHead(nc, reg_max, hidc, clsc, ch)
            args.extend([16, 64, 64])  # pad reg_max, hidc, clsc defaults
            args.insert(4, [ch[x] for x in f])  # insert ch at index 4
